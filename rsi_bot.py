...
def run_trading_cycle(api, paper: bool = True):
    ...
    if state.get("position"):
        ...
        try:
            ...
            _save_dashboard_state(state, portfolio_value)
            else:
    ...
    try:
        ...
        _add_log(state, f"✓ COMPRA {target['symbol']} @ ${target['price']:.2f} | RSI: {target['rsi']:.1f} | TP: ${tp_price} | SL: ${sl_price}")
...
def _save_dashboard_state(state: dict, portfolio_value: float):
    ...
    dashboard_state = {
        ...
        "total_pnl_pct": round((portfolio_value - CAPITAL) / CAPITAL * 100, 2),
        "win_rate": round(len(wins) / len(trades) * 100, 1) if trades else 0,
        "trades_today": sum(1 for t in trades if t.get("date") == str(date.today())),
        ...
        "closed_trades": trades[-20:],
        "last_updated": datetime.now().isoformat(),
        "cycle_log": state.get("cycle_log", []),
    }
    dashboard_file = DATA_DIR / "state.json"
    dashboard_file.write_text(json.dumps(dashboard_state, indent=2, default=str))


def _add_log(state: dict, msg: str):
